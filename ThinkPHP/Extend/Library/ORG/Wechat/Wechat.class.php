...
class Wechat
{
    ...
    const MEDIA_FOREVER_UPLOAD_URL = '/material/add_material?';//上传永久素材接口
    const MEDIA_FOREVER_NEWS_UPLOAD_URL = '/material/add_news?';//永久图文素材接口
...
// 		setLog($postStr);
	...
	*/
	 ...
	 * @return string content
	 */
	private function http_post2($url,$param,$post_file = false,$img_path = ''){
		$oCurl = curl_init();
		if(stripos($url,"https://")!==FALSE){
			curl_setopt($oCurl, CURLOPT_SSL_VERIFYPEER, FALSE);
			curl_setopt($oCurl, CURLOPT_SSL_VERIFYHOST, false);
...
//              fclose($fp);
	...
	public function uploadImg($data,$img_path = ''){
		...
		$result = $this->http_post2(self::API_URL_PREFIX.self::MEDIA_UPLOADIMG_URL.'access_token='.$this->access_token,$data,true,$img_path);
		if ($result)
		{
			$json = json_decode($result,true);
			if (!$json || !empty($json['errcode'])) {
	...
	/**
	 * 请注意该方法必须保证php的版本在5.6以下，否则会爆41005错误
	 */
	public function upload_meterial($file_info){
        if (!$this->access_token && !$this->checkAuth()) return false;
		 $url=self::API_URL_PREFIX.self::MEDIA_FOREVER_UPLOAD_URL.'access_token='.$this->access_token.'&type=image';
		 $ch1 = curl_init();
		 $timeout = 5;
